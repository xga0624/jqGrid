...
$.jgrid.extend({
	...
	setTableFrozenColumns : function () {
		return this.each(function() {
			if ( !this.grid ) {return;}
			var $t = this, cm = $t.p.colModel,i=0, len = cm.length, maxfrozen = -1,maxleftfrozen = 0, maxrightfrozen = 0, frozen= false,leftfrozen= false,rightfrozen= false,leftarr=[],rightarr=[];
			// TODO treeGrid and grouping  Support
			if($t.p.subGrid === true || $t.p.treeGrid === true || $t.p.cellEdit === true || $t.p.sortable || $t.p.scroll )
			{
				return;
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.leftfbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) +1 )+'px;overflow-y:hidden;    z-index: 11;" class="frozen-bdiv ui-jqgrid-bdiv left-frozen-bdiv"></div>');
				$t.grid.rightfhDiv = $('<div style="position:absolute;right:0px;top:1px; " class="frozen-div ui-state-default ui-jqgrid-hdiv right-frozen-hdiv"></div>');
				$t.grid.rightfbDiv = $('<div style="position:absolute;right:0px;top:'+(parseInt(top,10)+parseInt(hth,10)+1)+'px;overflow-y:hidden;  z-index: 11;" class="frozen-bdiv ui-jqgrid-bdiv right-frozen-bdiv"></div>');
				...
				$($t).bind('jqGridAfterGridComplete.setTableFrozenColumns', function () {
					$("#"+$.jgrid.jqID($t.p.id)+"_frozen").remove();
					// $("#"+$.jgrid.jqID($t.p.id)+"_leftfrozen").remove();
